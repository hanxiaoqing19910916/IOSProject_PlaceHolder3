...
{
    NSString *passwordRegex = @"^[a-zA-Z_0-9]{6,16}+$";
...
{
    NSString *userNameRegex = @"^[a-zA-Z_0-9]{4,10}+$";
...
//验证是否为空字符串
+ (BOOL)verifyEmptyString:(NSString *)emptyString{
    //1.是不是String
    if (![emptyString isKindOfClass:[NSString class]] && ![emptyString isKindOfClass:[NSMutableString class]]) {
        return false;
    }//2.length
